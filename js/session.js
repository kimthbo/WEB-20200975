...
function session_join_set(){ //세션 저장(객체)    
    ...
    if (sessionStorage) {
        ...
        let en_text = encrypt_text(objString); // 암호화
        sessionStorage.setItem("Session_Storage_Join_object", objString);
        sessionStorage.setItem("Session_Storage_Join_encrypted", en_text);
    } else {
        alert("세션 스토리지 지원 x");
    }   
}
...
function session_join_get() { //세션 읽기
    if (sessionStorage) {
       return sessionStorage.getItem("Session_Storage_Join_encrypted");
    } else {
        alert("세션 스토리지 지원 x");
    }
}